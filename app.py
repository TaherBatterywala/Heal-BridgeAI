...
def predict():
    disease_name = None  # Initialize variable for prediction result
    if request.method == "POST":
        # Collect the selected symptoms from the form
        selected_symptoms = [symptom for symptom in all_symptoms if request.form.get(symptom)]

        # Create an input feature vector with all symptoms
        ...
        predicted_index = int(prediction[0])  # already an index from 0-41

        # Decode to disease name (assuming it's mapped correctly in your model)
        disease_name = encoder.categories_[0][predicted_index]  # Adjust accordingly

    return render_template("index.html", categorized_symptoms=categorized_symptoms, prediction=disease_name)
